      ...
      <Reveal>
        <div className="lc-edge" style={{ ...glassPanel, borderRadius:24, padding:"26px 30px", marginBottom:22, position:"relative", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:16, flexWrap:"wrap" }}>
            <span style={{ color:ph.color, fontWeight:900, fontSize:13, display:"inline-flex", alignItems:"center", gap:7 }}><Icon name={ph.ic} size={15} />{ph.name}</span>
            <span style={{ color:FAINT, fontSize:12 }}>Stage <b style={{ color:TEXT }}>{stage.stage}</b> of 16</span>
            <div style={{ flex:1, minWidth:160, height:6, background:"var(--panel-2)", borderRadius:6, overflow:"hidden" }}>
              ...
              return (
                <button key={p.id} onClick={() => first && select(first.id)} className="lc-meshhost lc-shine"
                  style={{ position:"relative", overflow:"hidden", display:"inline-flex", alignItems:"center", gap:7, cursor:"pointer",
                    background: on ? `${p.color}22` : "var(--panel)",
                    border:`1px solid ${on ? p.color + "88" : "var(--hairline)"}`,
            ...
            return (
              <div key={p.id} style={{ marginBottom:6 }}>
                <div style={{ display:"flex", alignItems:"center", gap:8, margin:"12px 2px 8px" }}>
                  <span style={{ display:"flex", color:p.color }}><Icon name={p.ic} size={14} /></span>
                    ...
                    return (
                      <button key={s.id} onClick={() => select(s.id)} className="lc-meshhost lc-shine"
                        ...
                        <span style={{ position:"relative", zIndex:1, flexShrink:0, width:30, height:30, borderRadius:"50%",
                          ...
                          color: on ? "#06121b" : SUB,
                          background: on ? `radial-gradient(circle at 35% 28%, #fff, ${s.accent} 70%)` : "var(--panel-2)",
                          border:`1.5px solid ${on ? s.accent : "var(--hairline)"}`,
                          boxShadow: on ? `0 0 14px ${s.accent}, inset 0 1px 0 rgba(255,255,255,.7)` : "none",
        ...
        <div ref={detailRef} className="lc-glass lc-edge" style={{ borderRadius:22, padding:"24px 26px 26px", position:"relative", overflow:"hidden", animation:"lc-dropin .25s ease" }}>
          ...
          <div style={{ display:"flex", gap:16, alignItems:"center", marginBottom:18, flexWrap:"wrap" }}>
            <span style={{ fontSize:34, width:62, height:62, borderRadius:18, flexShrink:0, display:"flex", alignItems:"center", justifyContent:"center",
              ...
              boxShadow:`inset 0 1px 0 rgba(255,255,255,.5), 0 10px 24px -8px ${stage.accent}aa`, color:"#fff" }}><Icon name={stageIcon(stage.id)} size={28} /></span>
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              ...
              <span style={{ background:"var(--panel)", color:SUB, border:"1px solid var(--hairline)", borderRadius:20, padding:"4px 12px", fontSize:11, fontWeight:700 }}>{stage.questions?.length || 0} questions</span>
            </div>
          </div>
              ...
              {overview.steps?.length > 0 && (
                <>
                  ...
                  {overview.steps.map((step, i) => (
                    <div key={i} style={{ display:"flex", gap:14, background:"var(--panel)", border:"1px solid var(--hairline)",
                  ...
                  return (
                    <button key={l} onClick={() => setLvl(l)}
                      style={{ cursor:"pointer", borderRadius:999, padding:"5px 13px", fontSize:11.5, fontWeight:700,
                        background: on ? (l === "All" ? "var(--panel-2)" : `${c}26`) : "var(--panel)",
                        border:`1px solid ${on ? (l === "All" ? "var(--border-bright)" : c + "88") : "var(--hairline)"}`,
          ...
          {/* ── Prev / Next ── */}
          <div style={{ display:"flex", justifyContent:"space-between", gap:12, marginTop:24, paddingTop:18, borderTop:"1px solid var(--hairline)" }}>
            {prev ? (
